...
[![Version 6.58.3](https://img.shields.io/badge/version-6.58.3-green.svg)](VERSION)
...
| 6.58.1 | 2026-07-09 | **fix: scroll-to-bottom button icon uses the accent-light design token.** Follow-up to the v6.58.0 blocking review — the new `.chat-scroll-bottom-btn` icon color was a one-off hex (`#e9b7be`); it now uses `var(--accent-light)` so the control stays on the design-system SSOT (no new crimson shade). No behavior change. |
...
| 6.57.3 | 2026-07-09 | **fix: delivered-document reload edge cases from the v6.57.2 blocking review.** Two follow-up fixes on the just-shipped delivered-file work: (1) the `syncHistory` document branch is hoisted above the `taskId → finishLiveCard` block so a mid-task file delivery replayed while its task is still running no longer falsely finalizes that task's live card; (2) `web/modules/chat.js` deduplicates the document bubble via `seenMessageKeys` keyed on the shared `ts` (the live WS insert and the persisted-row history replay carry one identical `ts`), so a routine background sync never re-inserts an already-rendered file bubble. Also drops a redundant local `quote` import in `download_url_for_local_file`. |
| 6.57.2 | 2026-07-09 | **fix: delivered files are downloadable in the desktop app and survive reload.** Two owner-reported bugs with agent-delivered chat files: (1) clicking a delivered PDF in the macOS desktop app (pywebview/WKWebView ignores `<a download>`) navigated the WebView fullscreen and locked the UI; (2) the delivered-file message vanished after reopening because `send_document` never persisted to `chat.jsonl` and `/api/chat/history` had no media replay. `send_file` now copies the file into the task's canonical artifact store and derives a loopback `/api/files/download?path=` URL from that durable copy (new `download_url_for_local_file` helper, root-relative because `safe_relpath` lstrips `/`); `DocumentOutbound` carries an optional `download_url` (frozen contract, pro), and the chat.js document bubble routes downloads through `downloadViaHostBridge` (desktop host-bridge saves to Downloads instead of navigating; browser falls back to fetch+blob). `send_document` persists a compact `chat.jsonl` row (no base64) via `log_chat(record_type="document", …)`, and `/api/chat/history` replays it as a `msg_type="document"` bubble rebuilt from the durable URL. NOT included (reported as separate follow-ups): photo/video reload persistence (no on-disk path) and inbound Telegram-message persistence (a review-gated skill change). |
