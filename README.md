...
[![Version 6.15.8](https://img.shields.io/badge/version-6.15.8-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.15.8 | 2026-06-05 | **patch(widgets): stack declarative progress widget layout.** Splits the declarative `progress` component in `web/modules/widgets.js` into a `.widget-progress-bar-row` (HTML5 progress bar + crimson pulse on one horizontal line) with the stage/status label moved to a `.widget-progress-label` block beneath it, so long stage descriptions (e.g. anime_studio generation stages) no longer crowd the bar. `web/style.css` makes `.widget-progress` a vertical flex column, stretches the bar to full width, and wraps long labels; `tests/test_widgets_ui_static.py` asserts the new row/label structure in both markup and CSS. |
| 6.15.7 | 2026-06-04 | **patch(widgets): fix local file downloads in desktop Webview under VPN/proxy.** Configures `download_file_to_downloads` inside `launcher.py` to use a proxy-free urllib opener (`ProxyHandler({})`) when requesting localhost download artifacts. This forces local loopback download requests to bypass any system-active VPN or proxy configurations, preventing 502 Bad Gateway errors and avoiding Webview navigation traps. |
| 6.15.6 | 2026-06-04 | **patch(widgets): remove hard-coded progress bar gradients.** Integrates a new `--accent-hover` CSS custom property inside stylish `:root` definitions in `web/style.css`. Migrates the custom progress fill gradients to dynamically reference those accent token variables instead of hard-coded palette fallbacks to prevent token drift. |
...
Older releases are preserved in Git tags and GitHub releases. The 6.0.0 through 6.11.0-rc.1 rows, the 6.7.x patch rows, the 6.15.3 patch row, the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
