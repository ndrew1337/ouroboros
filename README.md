...
[![Version 6.58.2](https://img.shields.io/badge/version-6.58.2-green.svg)](VERSION)
...
| 6.57.1 | 2026-07-08 | **fix: enumerate `chat.document` in the Skill Review Checklist.** Addresses the v6.57.0 scope-review finding: `chat.document` became a subscribable event topic (`event_bus.VALID_TOPICS`) carrying owner-delivered file bytes, but the Skill Review Checklist item 10 (`event_subscription_minimization`) sensitive-topic enumeration was not updated in lockstep with its sibling `chat.photo`/`chat.video`. A skill subscribing to `chat.document` would otherwise slip past the hard trust-boundary review item. Adds `chat.document` (and the "including delivered file bytes" note) to `docs/CHECKLISTS.md` item 10 so the upcoming telegram-bridge `chat.document` subscription is governed correctly. |
